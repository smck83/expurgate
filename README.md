...
| expurgate-resolver  | RUNNING_CONFIG_ON= | When set to `1`, generate a single conf file called `running-config` for all domains, instead of a config file per domain. The main benefit is that expurgate-rbldnsd doesnt have to be restarted to learn about new files.  | N |
