...
Expurgate is a passion project that provides the capability to host your own SPF flattening solution that leverages SPF macro's. There is no webUI and no database - these may be added as an option in future versions. The entire configuration is completed using ENV variables parsed at runtime. This solution will resolve your old SPF records moved to a TXT DNS record published on an unused subdomain that will act as the source of truth and is how you will make changes when you need to add or remove IP, subnets and hostnames - Expurgate will detect these changes and publish the new rbldnsd configuration. In the example below the subdomain `_sd6sdyfn.yourdomain.com` is used. A script running on the spf-resolver container will loop through these records (no issues if they exceed 10 lookups or are duplicates) and generate an rbldnsd configuration file of IP addressess and subnets that rbldnsd will use to update.
